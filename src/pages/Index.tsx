        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-0">
          <div className={`transition-transform duration-100 ${glitchActive ? 'animate-glitch' : ''} text-center sm:text-left`}>
            <h1 className="text-2xl sm:text-3xl font-bold text-[#3b00ff]">
              {currentUser && currentUser.name === 'Unknown Entity' ? 'ПРОТОКОЛ СПАСЕНИЯ ЖЕРТВ' : 'REQN CO LABORATORY'}
            </h1>
            <p className="text-xs sm:text-sm opacity-70 text-[#24ff00]">
              {currentUser && currentUser.name === 'Unknown Entity' ? 'СИСТЕМА ОСВОБОЖДЕНИЯ v∞.∞.∞' : 'СИСТЕМА БЕЗОПАСНОСТИ v2.1.3'}
            </p>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <h3 className="text-lg font-mono flex items-center gap-2 text-[#db14b5]">
                  ...
                  {currentUser && currentUser.name === 'Unknown Entity' ? 'ЖЕРТВЫ - Требуют немедленного спасения' : 'LOWER - Безопасные существа'}
                </h3>
                  ...
                  {artificialBeings.lower.map((being) => (
                    <Card key={being.id} className="bg-vhs-black border-green-400">
                      ...
                      <CardContent className="pt-0">
                        ...
                        >
                          <Icon name="FileText" size={14} className="mr-1" />
                          {currentUser && currentUser.name === 'Unknown Entity' ? 'ПЛАН СПАСЕНИЯ' : 'ПОДРОБНЕЕ'}
                        </Button>
                      </CardContent>
                    </Card>
                  ))}
              ...
              <div className="space-y-3">
                <h3 className="text-lg font-mono flex items-center gap-2 text-[#addb14]">
                  ...
                  {currentUser && currentUser.name === 'Unknown Entity' ? 'СТРАДАЮЩИЕ ЖЕРТВЫ - В опасности' : 'MEDIUMNESS - Опасные существа'}
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {artificialBeings.mediumness.map((being) => (
                    <Card key={being.id} className="bg-vhs-black border-yellow-400">
                      ...
                      <CardContent className="pt-0">
                        ...
                        >
                          <Icon name="FileText" size={14} className="mr-1" />
                          {currentUser && currentUser.name === 'Unknown Entity' ? 'ОЦЕНИТЬ УГРОЗУ' : 'ПОДРОБНЕЕ'}
                        </Button>
                      </CardContent>
                    </Card>
                  ))}
              ...
              <div className="space-y-3">
                <h3 className="text-lg font-mono flex items-center gap-2 text-[#000ecc]">
                  ...
                  {currentUser && currentUser.name === 'Unknown Entity' ? 'КРИТИЧЕСКАЯ ОПАСНОСТЬ - Освободить СРОЧНО!' : 'DANGERER - Смертельно опасные существа'}
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <Card key={being.id} className="bg-vhs-black border-vhs-red">
                      ...
                      <CardContent className="pt-0">
                        ...
                        >
                          ...
                          {currentUser && currentUser.name === 'Unknown Entity' ? 'ОСВОБОДИТЬ!' : 'КРИТИЧНО'}
                        </Button>
                      </CardContent>
                    </Card>
                  ))}
              ...
              <div className="space-y-6">
                <div className="text-center mb-6">
                  <h2 className="text-2xl font-mono text-purple-400 animate-pulse mb-2">
                    {currentUser && currentUser.name === 'Unknown Entity' ? 'ОПЕРАЦИЯ "ОСВОБОЖДЕНИЕ" - СПАСТИ ВСЕХ!' : 'НЕИЗВЕСТНЫЕ И НЕИЗУЧЕННЫЕ RQ'}
                  </h2>
                  <p className="text-sm opacity-70 text-purple-300">
                    {currentUser && currentUser.name === 'Unknown Entity' ? 'ДОСТУП К СЕКРЕТНЫМ УСТРОЙСТВАМ И ЖЕРТВАМ' : 'ДОСТУП ПОЛУЧЕН ПО СЕКРЕТНОМУ КОДУ'}
                  </p>
